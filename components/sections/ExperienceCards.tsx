...
function TeacherMockup() {
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E5E5", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
      ...
      <div className="flex gap-2 px-3 py-3" style={{ borderTop: "1px solid #F0F0F0" }}>
        <div className="flex-1 text-xs font-semibold py-2 rounded-lg text-center" style={{ backgroundColor: "#F79863", color: "white" }}>
          + Log observation
        </div>
        <div className="flex-1 text-xs font-semibold py-2 rounded-lg text-center" style={{ backgroundColor: "#F0F0F0", color: "#666666" }}>
          View reports
        </div>
      </div>
    </div>
...
function StudentMockup() {
  return (
    <div className="rounded-xl overflow-hidden" style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E5E5", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
      ...
      <div className="p-4">
        <div className="rounded-xl p-4 text-center mb-3" style={{ backgroundColor: "#FFF8E8", border: "1px solid #FDE8B0" }}>
        ...
        <p className="text-xs font-semibold mb-1.5 px-0.5" style={{ color: "#999999" }}>Up next</p>
        {[
          { emoji: "🔢", label: "Count to 10", area: "NUM", color: "#7BA3D4", bg: "#EAF2FB" },
          { emoji: "🌟", label: "Share & take turns", area: "SED", color: "#E8745A", bg: "#FEF0EC" },
        ].map((act) => (
          <div key={act.label} className="flex items-center gap-2.5 px-3 py-2 rounded-lg mb-1.5" style={{ backgroundColor: act.bg, border: `1px solid ${act.color}30` }}>
            <span className="text-base" aria-hidden="true">{act.emoji}</span>
            <span className="text-xs font-semibold" style={{ color: "#333333" }}>{act.label}</span>
            <span className="ml-auto text-xs font-semibold px-1.5 py-0.5 rounded-full" style={{ backgroundColor: act.color + "25", color: act.color }}>{act.area}</span>
          </div>
        ))}
      </div>
    </div>
...
function ParentMockup() {
  return (
    <div className="rounded-xl overflow-hidden" style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E5E5", boxShadow: "0 2px 16px rgba(0,0,0,0.06)" }}>
      <div className="px-4 pt-3 pb-3" style={{ borderBottom: "1px solid #F0F0F0" }}>
      ...
      <div className="px-4 py-3" style={{ backgroundColor: "#FAFAFA" }}>
        <p className="text-xs font-semibold mb-2" style={{ color: "#333333" }}>This week</p>
        {[
          ...
          <div key={area.label} className="mb-1.5">
            <div className="flex justify-between mb-0.5">
              <span style={{ color: "#999999", fontSize: 10 }}>{area.label}</span>
              <span className="font-semibold" style={{ color: area.color, fontSize: 10 }}>{area.value}%</span>
            </div>
      ...
      <div className="px-4 py-3 space-y-2">
        <p className="text-xs font-semibold" style={{ color: "#333333" }}>Latest updates</p>
        {[
          { emoji: "🌟", text: "Aiden achieved Secure on Letter Sounds", time: "2m ago", bg: "#E5F4F1" },
          { emoji: "🔢", text: "Aiden completed Count to 20", time: "Yesterday", bg: "#EAF2FB" },
        ].map((u, i) => (
...
export default function ExperienceCards() {
  return (
    <section aria-label="Who Nurture is for" className="py-24" style={{ backgroundColor: "#F5F5F5" }}>
